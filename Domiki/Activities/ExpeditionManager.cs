...
using Expedition = Domiki.Web.Activities.Models.Expedition;
...
public class ExpeditionManager
{
    ...
    public ExpeditionState? GetExpeditions(int playerId)
    {
        ...
        var active = _context.Expeditions.Where(x => x.PlayerId == playerId)
    ...
    public bool FinishExpedition(DateTime date, CalculateInfo calcInfo)
    {
        ...
        _playerEventManager.Record(calcInfo.PlayerId, PlayerEventType.ExpeditionReturned, new { expeditionTypeId = dbExpedition.ExpeditionTypeId, loot });
    ...
    public object ApplyLootEntry(int playerId, ExpeditionType type, ExpeditionLoot entry, Worker[] squadWorkers, Dictionary<int, Trait> traits, int groupLuck)
    {
        switch (entry.Kind)
        {
            ...
            case ExpeditionLootKind.Blueprint:
    ...
    private object ApplyTraitUpgrade(int playerId, ExpeditionType type, Worker[] squadWorkers, Dictionary<int, Trait> traits, int groupLuck, bool isRare)
    {
        ...
        {
            var fallbackPool = type.Loot.Where(x => x.IsRare && x.Kind != ExpeditionLootKind.TraitUpgrade).ToArray();
        ...
        return new { kind = (int)ExpeditionLootKind.TraitUpgrade, workerName = worker.Name, workerGender = (int)NameGrammar.GenderOf(worker.Name), newTrait = newTrait.Name, newTraitLogicName = newTrait.LogicName, isRare };
    }

    private object ApplyBlueprintLoot(int playerId, ExpeditionType type, ExpeditionLoot entry, Worker[] squadWorkers, Dictionary<int, Trait> traits, int groupLuck, bool isRare)
    {
        ...
        if (candidates.Length == 0)
        {
            var fallbackPool = type.Loot
                .Where(x => x.IsRare && x.Kind != ExpeditionLootKind.Blueprint && x.Kind != ExpeditionLootKind.TraitUpgrade)
                .ToArray();
        ...
        return new { kind = (int)ExpeditionLootKind.Blueprint, blueprintId = blueprint.Id, blueprintName = blueprint.Name, isRare };
    }
