...
using Order = Domiki.Web.Economy.Models.Order;
...
public class GoalsTests : TestBase
{
    ...
    public void BarracksUpgradeCompletesFifthGoalAfterEarlierGoalsTest()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(GetCompletedGoalIds(playerId), Is.EqualTo([1, 2, 3, 4, 5]));
    ...
    public void FirstGoalCompletesOnClayDigAndWritesJournalEntryTest()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(GetCompletedGoalIds(playerId), Is.EqualTo([1]));
    ...
    public void SaleBeforeItsActivationNeedsSecondSaleToCompleteThirdGoalTest()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(GetCompletedGoalIds(playerId), Is.EqualTo([1, 2]));
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(GetCompletedGoalIds(playerId), Is.EqualTo([1, 2, 3]));
    ...
    private GoalsState GetGoalsState(int playerId)
    {
        using var uow = GetUow();
    ...
    private void CompleteClayDigGoal(int playerId)
    {
        StartManufacture(playerId, ClayMineDomikId, ClayDigReceiptId);
    }
    ...
    {
        using var uow = GetUow();
        uow.Context.PlayerGoals.AddRange(goalIds.Select(goalId => new PlayerGoal
        {
            PlayerId = playerId,
            GoalId = goalId,
