...
using Worker = Domiki.Web.Workers.Models.Worker;
...
public class ExpeditionTests : TestBase
{
    ...
    public void ApplyBlueprintLootFallsBackWhenAllOwnedTest()
    {
        ...
        var result = manager.ApplyLootEntry(playerId, type, entry, [], traits, 0);
        uow.Commit();
    ...
    public void ApplyLootEntryDecorGrantsPlayerDecorTest()
    {
        ...
        using (var uow = GetUow())
        {
            ...
            manager.ApplyLootEntry(playerId, type, entry, [], traits, 0);
            uow.Commit();
        }
    ...
    public void FootScoutStartsWithoutGoldRowTest()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(ResourceValue(GetResources(playerId), GoldResourceTypeId), Is.Zero);
        }
    }
    ...
    public void PityThresholdForcesRareLootOnLongJourneyAndResetsCounterTest()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(GetPityCounter(playerId), Is.Zero);
        }
    }
    ...
    public void StartExpeditionWithBusyChosenWorkerThrowsTest()
    {
        ...
        StartManufacture(playerId, 6, 1, [workerIds[0]]);

        var ex = Assert.Throws<BusinessException>(() => StartExpedition(playerId, ShortScoutId, [workerIds[0], workerIds[1]]));
    ...
    public void StartExpeditionWithDuplicateWorkerThrowsTest()
    {
        ...
        var ex = Assert.Throws<BusinessException>(() => StartExpedition(playerId, ShortScoutId, [workerId, workerId]));
    ...
    public void StartExpeditionAssignsWorkersAndWritesOffGoldTest(int expeditionTypeId, int workerCount, int goldCost, int durationSeconds)
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(ResourceValue(resources, GoldResourceTypeId), Is.Zero);
            Assert.That(ResourceValue(resources, PlankResourceTypeId), Is.Zero);
        }
    }
    ...
    private static IEnumerable<TestCaseData> InsufficientWorkerCases()
    {
        ...
        {
            test.StartManufacture(playerId, 2, 1, [workerIds[0]]);
            test.StartManufacture(playerId, 5, 1, [workerIds[1]]);
