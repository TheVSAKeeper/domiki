...
public sealed class VillageTests
{
    ...
    {
        var takenName = "Тихий Бор-" + Guid.NewGuid().ToString("N")[..6];
        var keptName = "Ясная Поляна-" + Guid.NewGuid().ToString("N")[..6];

        var first = TestPlayer.Create()
            .SetVillageIdentity(takenName, 1, 2);

        var second = TestPlayer.Create()
            .SetVillageIdentity(keptName, 3, 4);

        Assert.Throws<BusinessException>(() => second.SetVillageIdentity(takenName, 5, 6));

        var village = second.Village();
        using (Assert.EnterMultipleScope())
        {
    ...
    {
        var name = "Тихая Долина-" + Guid.NewGuid().ToString("N")[..6];

        var player = TestPlayer.Create()
            .SetVillageIdentity("  " + name.Replace(" ", "   ") + "  ", 2, 3);
    ...
    public void SetVillageInvalidNameThrowsTest(string name)
    {
        ...
        Assert.Throws<BusinessException>(() => player.SetVillageIdentity(name, 0, 0));

        Assert.That(player.Village().VillageName, Is.Null);
    }
    ...
    public void SetVillageInvalidCrestThrowsTest(int crestIcon, int crestColor)
    {
        ...
        Assert.Throws<BusinessException>(() => player.SetVillageIdentity("Берёзки", crestIcon, crestColor));
