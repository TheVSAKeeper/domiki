...
public class TestBase
{
    ...
    public void MuteFtue(int playerId)
    {
        ...
        uow.Context.PlayerGoals.AddRange(uow.Context.StarterGoals.Select(x => x.Id)
            .ToArray()
    ...
    public DomikManager GetDomikManager(UnitOfWork uow, bool calculatorJustFinishMode = true)
    {
        ...
        var domikManager = new DomikManager(uow, uow.Context, GetCalculator(calculatorJustFinishMode), resourceManager, playerResourceManager, workerManager, weatherManager, villageLevelCalculator,
            blueprintManager, tolokaManager, playerEventManager, goalManager);
    ...
    public WorkerManager GetWorkerManager(UnitOfWork uow)
    {
        var resourceManager = new ResourceManager(uow.Context);
        var playerResourceManager = new PlayerResourceManager(uow.Context, resourceManager);
        return new(uow.Context, resourceManager, playerResourceManager);
    }
    ...
    public OrderManager GetOrderManager(UnitOfWork uow, bool calculatorJustFinishMode = false)
    {
        ...
        var tolokaManager = new TolokaManager(uow, uow.Context, resourceManager, playerResourceManager, GetSeasonManager(uow), playerEventManager, _broker);
        var goalManager = new GoalManager(uow.Context, resourceManager, playerResourceManager, villageLevelCalculator, playerEventManager);
        var orderManager = new OrderManager(uow, uow.Context, GetCalculator(calculatorJustFinishMode), resourceManager, playerResourceManager, workerManager, villageLevelCalculator, GetSeasonManager(uow),
            tolokaManager, goalManager);
    ...
    public VillageLevelCalculator GetVillageLevelCalculator(UnitOfWork uow)
    {
        ...
        var playerResourceManager = new PlayerResourceManager(uow.Context, resourceManager);
        var workerManager = new WorkerManager(uow.Context, resourceManager, playerResourceManager);
        return new(uow.Context, resourceManager, workerManager);
    }
    ...
    public ExpeditionManager GetExpeditionManager(UnitOfWork uow, bool calculatorJustFinishMode = true)
    {
        ...
        return new(uow,
            uow.Context,
            GetCalculator(calculatorJustFinishMode),
            resourceManager,
            playerResourceManager,
            workerManager,
            GetSeasonManager(uow),
            GetPlayerEventManager(uow),
            decorManager,
            GetBlueprintManager(uow));
    }
    ...
    public TolokaManager GetTolokaManager(UnitOfWork uow)
    {
        var resourceManager = new ResourceManager(uow.Context);
        var playerResourceManager = new PlayerResourceManager(uow.Context, resourceManager);
        return new(uow, uow.Context, resourceManager, playerResourceManager, GetSeasonManager(uow), GetPlayerEventManager(uow), _broker);
    }
    ...
    public MarketManager GetMarketManager(UnitOfWork uow, bool calculatorJustFinishMode = false)
    {
        var resourceManager = new ResourceManager(uow.Context);
        var playerResourceManager = new PlayerResourceManager(uow.Context, resourceManager);
        return new(uow, uow.Context, GetCalculator(calculatorJustFinishMode), resourceManager, playerResourceManager, GetPlayerEventManager(uow), _broker);
    }
    ...
    public DecorManager GetDecorManager(UnitOfWork uow)
    {
        var resourceManager = new ResourceManager(uow.Context);
        var playerResourceManager = new PlayerResourceManager(uow.Context, resourceManager);
        return new(uow, uow.Context, resourceManager, playerResourceManager);
    }
    ...
    public WorldManager GetWorldManager(UnitOfWork uow)
    {
        ...
        var tolokaManager = new TolokaManager(uow, uow.Context, resourceManager, playerResourceManager, GetSeasonManager(uow), playerEventManager, _broker);
        var goalManager = new GoalManager(uow.Context, resourceManager, playerResourceManager, villageLevelCalculator, playerEventManager);
        var domikManager = new DomikManager(uow, uow.Context, GetCalculator(), resourceManager, playerResourceManager, workerManager, weatherManager, villageLevelCalculator,
            blueprintManager, tolokaManager, playerEventManager, goalManager);

        return new(uow.Context, villageLevelCalculator, domikManager, resourceManager, GetSeasonManager(uow));
    }

    public WeatherManager GetWeatherManager(UnitOfWork uow, bool calculatorJustFinishMode = true)
    {
        var resourceManager = new ResourceManager(uow.Context);
        return new(uow.Context, uow, GetCalculator(calculatorJustFinishMode), resourceManager);
    }

    protected static string TestVillageName()
    {
        return TestVillageName(VillageNameRoots[Random.Shared.Next(VillageNameRoots.Length)]);
    }
    ...
    {
        return new TestCalculator(GetUow, uow => new(GetDomikManager(uow), GetOrderManager(uow), GetWeatherManager(uow), GetExpeditionManager(uow), GetMarketManager(uow)), justFinishMode);
    }
}
