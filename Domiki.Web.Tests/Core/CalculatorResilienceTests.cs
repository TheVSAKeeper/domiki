...
public class CalculatorResilienceTests
{
    ...
    public void DrainProcessesAllDueEventsInOnePassTest()
    {
        ...
        var due2 = new CalculateInfo
        {
            ObjectId = 2,
            Type = CalculateTypes.Manufacture,
            Date = baseDate.AddSeconds(1),
        };

        var due3 = new CalculateInfo
        {
            ObjectId = 3,
            Type = CalculateTypes.Manufacture,
            Date = baseDate.AddSeconds(2),
        };

        var future = new CalculateInfo
        {
            ObjectId = 4,
            Type = CalculateTypes.Manufacture,
            Date = baseDate.AddSeconds(1000),
        };

        var calc = new PoisonCalculator(poisonObjectId: -1);
        calc.SeedForTest([due1, due2, due3, future]);

        calc.DrainDue(baseDate.AddSeconds(10), 100);

        using (Assert.EnterMultipleScope())
        {
            Assert.That(calc.GoodProcessed, Is.EqualTo(3), "все наступившие события обработаны за один проход");
            Assert.That(calc.PendingForTest.Select(x => x.ObjectId), Is.EquivalentTo([4]), "будущее событие осталось нетронутым");
        }
    }
    ...
    public void DrainStopsAtBudgetTest()
    {
        ...
        calc.DrainDue(baseDate.AddSeconds(100), 2);
